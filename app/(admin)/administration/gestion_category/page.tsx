...
import { 
  ...
  X, Image as ImageIcon, Tag, LayoutGrid
} from "lucide-react"
...
export default function CategoryAdminPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) { setIsOpen(false); fetchCategories() }
    } catch (error) { console.error(error) } finally { setIsSubmitting(false) }
  }

  const handleDelete = async (id: string) => {
    setCategories(prev => prev.filter(c => c.id !== id))
    try { await fetch(`${CAT_API_URL}/${id}`, { method: "DELETE" }) } catch (e) { fetchCategories() }
  }
                    ...
                    >
                      <TableCell className="pl-6">
                        <div className="h-12 w-12 rounded-xl overflow-hidden bg-zinc-100 border border-zinc-200 shadow-sm">
                          <img src={cat.image || "/placeholder.png"} className="h-full w-full object-cover" />
                        </div>
                      </TableCell>
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent className="rounded-2xl border-none shadow-2xl">
                              ...
                              <AlertDialogFooter>
                                ...
                                <AlertDialogAction onClick={() => handleDelete(cat.id)} className="rounded-full bg-red-600 hover:bg-red-700">Confirm</AlertDialogAction>
                              </AlertDialogFooter>
                            </AlertDialogContent>
                          </AlertDialog>
                        </div>
        ...
        <DrawerContent className="rounded-t-[32px] border-none shadow-2xl">
          <div className="mx-auto w-full max-w-lg px-6 py-10 overflow-y-auto">
            <header className="flex justify-between items-center mb-10">
              ...
              <DrawerClose className="h-10 w-10 flex items-center justify-center bg-zinc-50 rounded-full">
                <X className="w-4 h-4" />
              </DrawerClose>
            </header>
            ...
            <div className="space-y-8">
              ...
              <div className="flex flex-col items-center justify-center gap-4">
                <div 
                  ...
                  className="h-40 w-full rounded-3xl bg-zinc-50 border-2 border-dashed border-zinc-200 flex items-center justify-center overflow-hidden cursor-pointer hover:bg-zinc-100 transition-all group"
                >
                  {formData.image ? (
